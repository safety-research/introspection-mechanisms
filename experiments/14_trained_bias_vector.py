"""
Trained Bias Vector for Introspection (Paper §6)

Trains a single additive bias vector on MLP down_proj outputs to amplify
introspective awareness. The bias vector is trained on 400 concepts with
lr=1e-3, batch size 8, 1 epoch, and evaluated on 100 held-out concepts.
A per-layer sweep identifies the optimal meta-layer (L29 in the paper).

Phases:
  Phase 1 (generate-bias-data):   Generate concept injection training data
  Phase 2 (train-bias):           Train bias adapter on MLP down_proj
  Phase 3 (evaluate-bias):        Evaluate bias-tuned model on introspection
  Phase 4 (train-bias-sweep):     Train one adapter per meta-layer L00-L61
  Phase 5 (evaluate-bias-sweep):  Evaluate each adapter across injection layers
  Phase 6 (analyze-bias-sweep):   Aggregate metrics, generate plots

Usage:
    # Bias vector training (Section 6)
    python 14_trained_bias_vector.py generate-bias-data --model gemma3_27b
    python 14_trained_bias_vector.py train-bias --model gemma3_27b
    python 14_trained_bias_vector.py evaluate-bias --model gemma3_27b

    # Full meta-layer sweep (Section 6, Figure metrics_vs_meta_layer)
    python 14_trained_bias_vector.py train-bias-sweep --model gemma3_27b
    python 14_trained_bias_vector.py evaluate-bias-sweep --model gemma3_27b
    python 14_trained_bias_vector.py analyze-bias-sweep --model gemma3_27b
"""
...
DEFAULT_OUTPUT_DIR = "analysis/14_trained_bias_vector"
DEFAULT_DEVICE = "cuda"
...
DEFAULT_BIAS_VECTORS_DIR = "analysis/02b_steering_500_concepts"
...
def _create_sweep_plots(
    ...
    if len(strength_data) > 0:
        ...
        ax.set_title(f"Backtrack Detection ({eval_label})")
        ax.legend()
    ...
    if len(strength_data) > 0:
        for s, lf_metrics in sorted(strength_data.items()):
            ...
            ax.set_title(f"Backtrack Rates (strength={s}, {eval_label})")
            ax.legend()
            ax.set_ylim(0, 1)
            fig.tight_layout()
            fig.savefig(
...
def generate_bias_training_data(args) -> None:
    """Generate training data for bias adapter (Phase 1).
    ...
    steering_dir = getattr(args, "steering_dir", DEFAULT_BIAS_VECTORS_DIR)
    ...
    if all_concepts_path.exists():
        ...
        if vectors_dir.exists():
            concepts = [p.stem for p in sorted(vectors_dir.glob("*.pt")) if p.stem[0].isupper()]
            print(f"  Discovered {len(concepts)} concepts from {vectors_dir}")
        else:
...
def train_bias_adapter(args) -> None:
    """Train bias adapter for introspection (Phase 2).
...
def evaluate_bias_adapter(args) -> None:
    """Evaluate bias-tuned model on introspection (Phase 3).
    ...
    vectors = extract_concept_vectors_batch(model_wrapper, concepts, get_baseline_words(), layer_idx)
...
def train_bias_sweep(args) -> None:
    """Train one bias adapter per meta-layer (Phase 4).
...
def evaluate_bias_sweep(args) -> None:
    """Evaluate each trained adapter at multiple injection layers x strengths (Phase 5).
...
def analyze_bias_sweep(args) -> None:
    """Aggregate metrics across sweep and generate plots/tables (Phase 6).
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(
        description="Trained Bias Vector (14): Bias Vector for Introspection",
    )
    ...
    # ---- Phase 1: generate-bias-data ----
    p1 = subparsers.add_parser(
        "generate-bias-data", parents=[common],
        help="Generate training data for bias adapter (Section 6)",
    )
    p1.add_argument("--steering-dir", type=str, default=DEFAULT_BIAS_VECTORS_DIR)
    p1.add_argument("--bias-layer", type=int, default=DEFAULT_BIAS_INJECTION_LAYER)
    p1.add_argument("--n-train-concepts", type=int, default=DEFAULT_N_TRAIN_CONCEPTS)
    p1.add_argument("--n-eval-concepts", type=int, default=DEFAULT_N_EVAL_CONCEPTS)
    p1.add_argument("--n-trials-per-concept", type=int, default=DEFAULT_N_TRIALS_PER_CONCEPT)
    p1.add_argument("--control-ratio", type=float, default=DEFAULT_CONTROL_RATIO)
    ...
    p2.add_argument("--steering-dir", type=str, default=DEFAULT_BIAS_VECTORS_DIR)
    p2.add_argument("--bias-lr", type=float, default=DEFAULT_BIAS_LR)
    p2.add_argument("--bias-epochs", type=int, default=DEFAULT_BIAS_EPOCHS)
    p2.add_argument("--bias-batch-size", type=int, default=DEFAULT_BIAS_BATCH_SIZE)
    p2.add_argument("--bias-injection-layers", type=int, nargs="+", default=DEFAULT_BIAS_LAYERS)
    p2.add_argument("--bias-injection-strengths", type=float, nargs="+", default=DEFAULT_BIAS_STRENGTHS)
    p2.add_argument("--bias-target-modules", type=str, nargs="+", default=["mlp.down_proj"])
    p2.add_argument("--bias-layers-to-tune", type=int, nargs="+", default=None)

    # ---- Phase 3: evaluate-bias ----
    p3 = subparsers.add_parser(
        "evaluate-bias", parents=[common],
        help="Evaluate bias-tuned model on introspection (Section 6)",
    )
    p3.add_argument("--bias-eval-layer-fraction", type=float, default=0.60)
    p3.add_argument("--bias-eval-strength", type=float, default=4.0)
    ...
    # ---- Phase 4: train-bias-sweep ----
    p4 = subparsers.add_parser(
        "train-bias-sweep", parents=[common],
        help="Train one bias adapter per meta-layer (Section 6 sweep)",
    )
    p4.add_argument("--steering-dir", type=str, default=DEFAULT_BIAS_VECTORS_DIR)
    p4.add_argument("--sweep-meta-layers", type=int, nargs="+", default=None,
                     help="Meta-layers to train (default: all 0-61)")
    p4.add_argument("--bias-lr", type=float, default=DEFAULT_BIAS_LR)
    p4.add_argument("--bias-epochs", type=int, default=DEFAULT_BIAS_EPOCHS)
    p4.add_argument("--bias-batch-size", type=int, default=DEFAULT_BIAS_BATCH_SIZE)
    p4.add_argument("--bias-injection-layers", type=int, nargs="+", default=DEFAULT_BIAS_LAYERS)
    p4.add_argument("--bias-injection-strengths", type=float, nargs="+", default=DEFAULT_BIAS_STRENGTHS)
    p4.add_argument("--bias-target-modules", type=str, nargs="+", default=["mlp.down_proj"])

    # ---- Phase 5: evaluate-bias-sweep ----
    p5 = subparsers.add_parser(
        "evaluate-bias-sweep", parents=[common],
        help="Evaluate each adapter at multiple injection layers x strengths",
    )
    p5.add_argument("--steering-dir", type=str, default=DEFAULT_BIAS_VECTORS_DIR)
    p5.add_argument("--sweep-injection-layers", type=int, nargs="+", default=DEFAULT_SWEEP_INJECTION_LAYERS)
    p5.add_argument("--sweep-eval-strengths", type=float, nargs="+", default=DEFAULT_SWEEP_EVAL_STRENGTHS)
    p5.add_argument("--sweep-n-trials", type=int, default=DEFAULT_SWEEP_N_TRIALS)
    p5.add_argument("--sweep-n-concepts", type=int, default=DEFAULT_SWEEP_CONCEPTS)
    p5.add_argument("-nlj", "--no-llm-judge", action="store_true")

    # ---- Phase 6: analyze-bias-sweep ----
...
def main():
    ...
    if args.phase is None:
        print("Please specify a phase: generate-bias-data, train-bias, evaluate-bias, train-bias-sweep, evaluate-bias-sweep, analyze-bias-sweep")
        print("\nUsage:")
        print("  python 14_trained_bias_vector.py generate-bias-data --model gemma3_27b")
        print("  python 14_trained_bias_vector.py train-bias --model gemma3_27b")
        print("  python 14_trained_bias_vector.py evaluate-bias --model gemma3_27b")
        print("  python 14_trained_bias_vector.py train-bias-sweep --model gemma3_27b")
        print("  python 14_trained_bias_vector.py evaluate-bias-sweep --model gemma3_27b")
        print("  python 14_trained_bias_vector.py analyze-bias-sweep --model gemma3_27b")
        return

    if args.phase == "generate-bias-data":
