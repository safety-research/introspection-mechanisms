...
DEFAULT_TOP_K = 10
DEFAULT_OUTLIER_THRESHOLD = 20.0
DEFAULT_TOKEN_POS = -1
...
def extract_steering_attribution(
    ...
    """Extract SA (GA, SG, SA) for all SAE features at one injection strength.
    ...
    for li in tqdm(range(n_layers), desc="Preloading SAEs", leave=False):
    ...
    def run_forward(s, retain_grad=False):
        ...
        hooks.register_hooks(list(range(n_layers)), CAPTURE_TYPES_SAE)
        try:
            with hooks:
                inner.eval()
    ...
    for li in range(n_layers):
        for j, suffix in enumerate(JVP_SITE_SUFFIXES):
            idx = li * len(JVP_SITE_SUFFIXES) + j
    ...
    with torch.no_grad():
        for li in tqdm(range(n_layers), desc="SA combine", leave=False):
            for st in SAE_TYPES:
                in_suffix, tgt_suffix = SAE_TYPE_KEYS[st]
    ...
    if feature_rows:
        pd.DataFrame(feature_rows).to_parquet(out_path, index=False)
        print(f"  Saved {len(feature_rows)} rows to {out_path}")
    torch.cuda.empty_cache()
    return out_path
...
def extract_feature_target_sa(
    ...
    """Extract feature-targeted SA where loss = target SAE feature activation.

    Same two-pass architecture as extract_steering_attribution, but the loss
    function is the activation of a specific target feature rather than the
    logit gap. This traces which upstream features drive the target.
    """
    import pandas as pd
    ...
    # Load target SAE
    target_load_type = SAE_TYPE_LOAD_MAP.get(target_sae_type, target_sae_type)
    target_sae = load_sae(target_layer, sae_width, sae_l0, target_load_type, model_size, True, device)

    # Preload upstream SAEs
    sae_cache: Dict[Tuple[int, str], JumpReLUSAE] = {}
    unique_types = set(SAE_TYPE_LOAD_MAP.get(st, st) for st in SAE_TYPES)
    for li in range(injection_layer, target_layer):
        for st in unique_types:
            try:
                sae = load_sae(li, sae_width, sae_l0, st, model_size, True, "cpu")
                sae_cache[(li, st)] = sae.eval()
    ...
    def make_hook(s):
        def hook(module, input, output):
            ...
            add[:, steering_start:, :] = (steering_vec * s).unsqueeze(0)
            return (h + add,) + rest if isinstance(output, tuple) else h + add
        return hook

    # Pass 1: Forward + backward (loss = target feature activation)
    handle = layers_module[injection_layer].register_forward_hook(make_hook(strength))
    hooks = ActivationHooks(inner, retain_grad=True)
    hooks.register_hooks(list(range(injection_layer, target_layer + 1)), CAPTURE_TYPES_SAE)
    try:
        with hooks:
            inner.eval()
            out = inner(input_ids=input_ids, attention_mask=attention_mask,
    ...
    # Get target site activation and compute loss
    raw_acts = hooks.get_activations()
    target_in_suffix = SAE_TYPE_KEYS[target_sae_type][0]
    target_act_key = _act_key(target_layer, target_in_suffix)
    if target_act_key not in raw_acts or not raw_acts[target_act_key]:
        return None
    target_act = raw_acts[target_act_key][0]
    if isinstance(target_act, (list, tuple)):
        target_act = target_act[0]
    target_act_2d = target_act[0] if target_act.dim() == 3 else target_act
    target_encoded = target_sae.encode(target_act_2d.float().to(device))
    tp = target_token_pos if target_token_pos >= 0 else seq_len + target_token_pos
    loss = target_encoded[tp, target_feature_id]
    loss.backward()
    ...
    for li in range(injection_layer, target_layer):
        for st in SAE_TYPES:
            in_suffix, tgt_suffix = SAE_TYPE_KEYS[st]
            ks = tgt_suffix if (st == "transcoder_all" and tgt_suffix) else in_suffix
            akey = _act_key(li, ks)
            if akey not in raw_acts or not raw_acts[akey]:
                continue
            t = raw_acts[akey][0]
            ...
            grad_data[(li, st)] = t.grad.detach().float()[0].cpu()
            enc_key = _act_key(li, in_suffix)
            if enc_key in raw_acts and raw_acts[enc_key]:
                enc_t = raw_acts[enc_key][0]
                if isinstance(enc_t, (list, tuple)):
                    enc_t = enc_t[0]
                act_data_local[(li, st)] = enc_t[0].detach().float().cpu()

    del raw_acts, out
    torch.cuda.empty_cache()

    # Pass 2: JVP for SG
    alpha_p = torch.tensor(strength, dtype=torch.float32, device=device)
    alpha_t = torch.tensor(1.0, dtype=torch.float32, device=device)

    def jvp_fwd(a):
        h = layers_module[injection_layer].register_forward_hook(make_hook(a))
        hks = ActivationHooks(inner, retain_grad=False)
        hks.register_hooks(list(range(injection_layer, target_layer)), CAPTURE_TYPES_SAE)
        try:
            with hks:
                inner.eval()
                _ = inner(input_ids=input_ids, attention_mask=attention_mask,
            ...
            h.remove()
            hks.remove_hooks()
        raw = hks.get_activations()
        result = []
        for li in range(injection_layer, target_layer):
            for sfx in JVP_SITE_SUFFIXES:
                ak = _act_key(li, sfx)
                t = raw[ak][0] if ak in raw and raw[ak] else torch.zeros(1)
                if isinstance(t, (list, tuple)):
                    t = t[0]
                result.append(t[0] if t.dim() == 3 else t)
        return result

    _, tangents = torch.func.jvp(jvp_fwd, (alpha_p,), (alpha_t,))

    tangent_data = {}
    for li_offset, li in enumerate(range(injection_layer, target_layer)):
        for j, sfx in enumerate(JVP_SITE_SUFFIXES):
            tangent_data[(li, sfx)] = tangents[li_offset * len(JVP_SITE_SUFFIXES) + j].detach()
    del tangents
    torch.cuda.empty_cache()
    ...
    base_meta = {
        ...
        "target_layer": target_layer, "target_sae_type": target_sae_type,
    ...
    with torch.no_grad():
        for li in range(injection_layer, target_layer):
            for st in SAE_TYPES:
                in_suffix, _ = SAE_TYPE_KEYS[st]
                lt = SAE_TYPE_LOAD_MAP.get(st, st)
                if (li, lt) not in sae_cache or (li, st) not in grad_data:
                    continue
                g = grad_data[(li, st)].to(device)
                x = act_data_local[(li, st)].to(device)
                sae = sae_cache[(li, lt)].to(device=device, dtype=torch.float32).eval()
                acts = sae.encode(x)
                ti, fi = (acts > 0).nonzero(as_tuple=True)
                n = len(ti)
                ga = sg = torch.zeros(n, device=device)
                if n > 0:
                    ga = (g[ti] * sae.w_dec[fi]).sum(-1)
                hp = li > injection_layer or (li == injection_layer and st == "resid_post_all")
                if hp and n > 0 and (li, in_suffix) in tangent_data:
                    dx = tangent_data[(li, in_suffix)].to(device).float()
                    if dx.dim() == 3:
                        dx = dx[0]
                    sg = (dx[ti] * sae.w_enc[:, fi].T).sum(-1)
                sa = ga * sg
                if n > 0:
                    for i in range(n):
                        rows.append({
                            **base_meta, "layer": li, "token_pos": ti[i].item(),
                            "sae_type": st, "feature_id": fi[i].item(),
                            "activation": acts[ti[i], fi[i]].item(),
                            "gradient_attribution": ga[i].item(),
                            "steering_grad": sg[i].item(),
                            "steering_attribution": sa[i].item(),
                        })
                sae.to("cpu")
                torch.cuda.empty_cache()
...
def select_positive_iqr_outliers(
    df, value_col: str, sae_type_col: str, threshold: float,
):
    """Per sae_type IQR outlier selection on positive values."""
    import pandas as pd

    df_pos = df[df[value_col] > 0]
    parts = []
    for st in sorted(df_pos[sae_type_col].unique()):
        st_df = df_pos[df_pos[sae_type_col] == st]
        vals = st_df[value_col]
        if len(vals) < 4:
            continue
        ...
        outliers = st_df[vals > fence]
        if not outliers.empty:
            parts.append(outliers)
            print(f"    {st}: {len(outliers)} outliers (fence={fence:.4f})")
    if not parts:
...
def select_top_features(
    ...
    """Select top features from ISA data using IQR outlier detection."""
    import pandas as pd
    ...
    top_df = select_positive_iqr_outliers(grouped, "isa_mean", "sae_type", outlier_threshold)
    if top_df.empty:
        print("  Warning: no outlier features found, taking top-10 by |ISA|")
...
def select_upstream_features(
    ...
    outlier_df = select_positive_iqr_outliers(weight_df, "weight", "sae_type", outlier_threshold)
...
def build_attribution_graph(
    ...
    hop0 = select_top_features(sa_dir, injection_layer, trial_nums, outlier_threshold)
    ...
    for hop in range(trace_depth):
        ...
        for target in current_targets:
            ...
            upstream, new_edges = select_upstream_features(
                ...
                visited, hop + 1, outlier_threshold * 10,
            )
...
def parse_args():
    ...
    p3.add_argument("--outlier-threshold", type=float, default=DEFAULT_OUTLIER_THRESHOLD)
    ...
    p5.add_argument("--outlier-threshold", type=float, default=DEFAULT_OUTLIER_THRESHOLD)
...
def main():
    ...
    if args.phase == "extract-sa":
        ...
        graph = build_attribution_graph(
            ...
            args.outlier_threshold, args.device)
